...
export const closeCartById = async (cart_id) => {
    const query = "UPDATE cart SET status = 'carrito_cerrado' WHERE cart_id = $1 RETURNING *";
    try {
...
export const incrementCartItem = async (detail_id, quantity) => {
    const query = "UPDATE cart_items SET quantity = quantity + $1 WHERE detail_id = $2 RETURNING *";
    try {
